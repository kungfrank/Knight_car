#!/usr/bin/env python
import rospy
#from duckietown_msgs.msg import Twisted2DStamped, ControlMode
from duckietown_msgs.msg import Twisted2DStamped, FSMState
...
#        self.cmd_dict[ControlMode.COORDINATION_CONTROL] = None
        ...
        self.cmd_dict[FSMState.JOYSTICK_CONTROL] = None
...
#        self.mode_name_dict[ControlMode.LANE_FOLLOWING] = "LANE_FOLLOWING"
 #       self.mode_name_dict[ControlMode.INTERSECTION_CONTROL] = "INTERSECTION_CONTROL"
  #      self.mode_name_dict[ControlMode.COORDINATION_CONTROL] = "COORDINATION_CONTROL"
        ...
        self.mode_name_dict[FSMState.JOYSTICK_CONTROL] = "JOYSTICK_CONTROL"

        # Setup publishers
        self.pub_car_cmd = rospy.Publisher("~car_cmd", Twisted2DStamped, queue_size=1)

        # Setup subscribers
        self.sub_mode = rospy.Subscriber("~mode", FSMState, self.cbMode, queue_size=1)
        self.sub_lane = rospy.Subscriber("~car_cmd_lane", Twisted2DStamped, self.cbCarCmd, queue_size=1, callback_args=FSMState.LANE_FOLLOWING)
        self.sub_interestion = rospy.Subscriber("~car_cmd_intersection", Twisted2DStamped, self.cbCarCmd, queue_size=1, callback_args=FSMState.INTERSECTION_CONTROL)
        self.sub_coordination = rospy.Subscriber("~car_cmd_coordination", Twisted2DStamped, self.cbCarCmd, queue_size=1, callback_args=FSMState.COORDINATION)
        self.sub_lane = rospy.Subscriber("~joy_control", Twisted2DStamped, self.cbCarCmd, queue_size=1, callback_args=FSMState.JOYSTICK_CONTROL)
    ...
    def pubCarCmd(self):
        ...
        self.pub_car_cmd.publish(cmd_msg)
    ...
    def cbCarCmd(self,cmd_msg,cb_args):
        # Save the cmd_msg 
