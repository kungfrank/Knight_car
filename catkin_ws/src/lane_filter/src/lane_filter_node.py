...
class LaneFilterNode(object):
    def __init__(self):
        ...
        self.cov_0  = [ [self.setupParam("~sigma_d_0",0),0] , [0, self.setupParam("~sigma_phi_0",0)] ] 
        ...
        self.d,self.phi = np.mgrid[d_min:d_max:delta_d,phi_min:phi_max:delta_phi]
        self.beliefRV=np.array()
        initializeBelief()
	...
	propagateBelief()
        ...
        for segment in segment_list_msg.segments:
            if segment.color != segment.WHITE && segment.color != segment.YELLOW:
                continue
            d_i,phi_i = generateVote(segment)
        ...
        updateBelief(measurement_likelihood)
    ...
    def initializeBelief(self):
        ...
        self.beliefRV=RV(pos)

    def propagateBelief(self):
        # starting with option 1 (don't read linear and angular velocity)
        # even simpler.. do nothing

    def updateBelief(self,ndarray):
        beliefRV=multiply(beliefRV,measurement_likelihood)
        beliefRV=beliefRV/np.linalg.norm(beliefRV)
