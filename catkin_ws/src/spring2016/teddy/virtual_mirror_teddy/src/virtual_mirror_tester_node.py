...
# Virtual Mirror Tester Node
# Author: Teddy Ort
# This node was created to test the functionality of the virtual_mirror_teddy

class VirtualMirrorTesterNode(object):
    def __init__(self):
        self.node_name = 'Virtual Mirror Tester'
        self.flip_direction   = self.setupParam("virtual_mirror_node/flip_direction", 'horz')
        self.orig_path        = self.setupParam("~orig_path", '../test_images/01_orig.png')
        self.horz_path        = self.setupParam("~horz_path", '../test_images/01_horz.png')
        self.vert_path        = self.setupParam("~vert_path", '../test_images/01_vert.png')

        self.sub = rospy.Subscriber("~image_in", Image, self.checkImage)
        self.pub = rospy.Publisher("~image_out", CompressedImage, queue_size=1, latch=True)
        ...
        self.pub.publish(self.image_orig_msg)
    ...
    def verifyResult(self):
        if self.result_vert is True and self.result_horz is True:
            ...
            rospy.set_param('virtual_mirror_node/flip_direction', new_dir)
...
if __name__ == '__main__':
    rospy.init_node('virtual_mirror_tester', anonymous=False)
